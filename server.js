...
const STAIRS_TIER_BAD_SHIFT = 0.05;    // wide-step complication chance grows (bonus chance shrinks) by this
                                        // much per tier — starts at an even 50/50 split, tier 1 (steps
                                        // 51-100) is 55/45, tier 2 is 60/40, and so on, capped so bonuses
                                        // never fully disappear. The farther a run goes, the more often
                                        // that every-10th-step roll lands on something bad instead of good.
...
const STAIRS_COLOR_CHANCES = { rainbow: 0.005, diamond: 0.05, gold: 0.10, black: 0.01, orange: 0.01, lime: 0.01, sticky: 0.002 }; // sums to 0.187, remaining 0.813 is a normal step (or a wide complication/bonus step)
const STAIRS_COLOR_BONUS = { gold: 0.05, diamond: 0.10, rainbow: 0.50, black: -0.10 }; // total multiplier contribution of that step (replaces the usual +0.01) — orange/lime/sticky aren't in here: orange/lime scale the running total instead, sticky doesn't touch the multiplier at all (see stairsApplyStepEffect)
const STAIRS_COLOR_LABELS = {
  ...
  sticky: 'Липкая ступень! Импульс погашен'
};

function stairsRollColor(){
  const r = Math.random();
  let acc = 0;
  for(const c of ['rainbow','diamond','gold','black','orange','lime','sticky']){
...
function stairsGenNext(g){
  ...
  if(isWide){
    const complicationChance = Math.min(STAIRS_TIER_BAD_CAP, 0.5 + tier*STAIRS_TIER_BAD_SHIFT);
