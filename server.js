...
const PLINKO_ROWS = 10;                 // must match the server's copy exactly
...
function plinkoBuildBoard(){
  ...
  const rowGapY = (bottomOfPegsY - topY) / (PLINKO_ROWS - 1);

  // Smaller pegs relative to the ball, and more importantly a clear
  // gap between them: gap between two same-row peg SURFACES is
  // slotW - 2*pegR, which needs to comfortably clear the ball's
  // diameter (2*ballR) for it to actually fit through.
  // Smaller pegs (thin dots, like the reference board) and a noticeably
  // bigger ball. Gap between two same-row peg SURFACES is slotW - 2*pegR
  // ≈ 0.90*slotW here, comfortably wider than the ball's own diameter
  // (2*ballR ≈ 0.68*slotW) — plenty of clearance for it to pass cleanly
  // between two pegs rather than clipping/tunneling through either one.
  const pegR = Math.max(2, slotW*0.05);
  const ballR = Math.max(6, slotW*0.34);

  // Low restitution is what makes this behave like a real Galton board
  // instead of a pinball machine: each peg should mostly just deflect the
  // ball a little left/right and let gravity keep pulling it down, not
  // send it ricocheting back up with most of its energy intact. High
  // restitution here was exactly why the ball used to bounce chaotically
  // toward the edges almost every drop instead of funneling toward the
  // (statistically far more likely) center columns.
  // restitution:0 — no bounciness at all. The ball's path down through the
  // triangle is now decided purely by which side of each peg it clips and
  // by gravity, not by any elastic kick back off a peg.
  const pegOpts = { isStatic:true, restitution:0, friction:0.1, label:'plinko_peg' };
  // Full 10-row triangle, apex row included — starts at 3 pegs (row 0) and
  // grows by 1 peg per row down to 12 pegs on the bottom row (row 9), so
  // the bottom row has exactly PLINKO_SLOTS-1 gaps lining up with the 11
  // slots below. Matches the reference board shape (3 pegs at the apex,
  // not 1 or 2).
  for(let r=0; r<PLINKO_ROWS; r++){
    const y = topY + r*rowGapY;
    const pegsInRow = r + 3;
    for(let k=0; k<pegsInRow; k++){
      const x = centerX + (k - (pegsInRow-1)/2)*slotW;
      const body = Bodies.circle(x, y, pegR, pegOpts);
      plinkoPegBodies.push(body);
    }
  }
...
const PLINKO_ROWS = 10;   // -> 11 slots (PLINKO_ROWS+1), indices 0..10
const PLINKO_MULTS = [0,1.5,2,3,5,10,5,3,2,1.5,0];
