...
const STAIRS_COLOR_CHANCES = { rainbow: 0.005, diamond: 0.05, gold: 0.10, black: 0.01, orange: 0.01 }; // sums to 0.175, remaining 0.825 is a normal step (or a wide complication/bonus step)
const STAIRS_COLOR_BONUS = { gold: 0.05, diamond: 0.10, rainbow: 0.50, black: -0.10 }; // total multiplier contribution of that step (replaces the usual +0.01) — orange isn't in here, it divides the running total instead (see stairsApplyStepEffect)
const STAIRS_COLOR_LABELS = {
  ...
  orange: 'Оранжевая ступень! Множитель ÷1.5'
};

function stairsRollColor(){
  const r = Math.random();
  if(r < STAIRS_COLOR_CHANCES.rainbow) return 'rainbow';
  if(r < STAIRS_COLOR_CHANCES.rainbow + STAIRS_COLOR_CHANCES.diamond) return 'diamond';
  if(r < STAIRS_COLOR_CHANCES.rainbow + STAIRS_COLOR_CHANCES.diamond + STAIRS_COLOR_CHANCES.gold) return 'gold';
  if(r < STAIRS_COLOR_CHANCES.rainbow + STAIRS_COLOR_CHANCES.diamond + STAIRS_COLOR_CHANCES.gold + STAIRS_COLOR_CHANCES.black) return 'black';
  if(r < STAIRS_COLOR_CHANCES.rainbow + STAIRS_COLOR_CHANCES.diamond + STAIRS_COLOR_CHANCES.gold + STAIRS_COLOR_CHANCES.black + STAIRS_COLOR_CHANCES.orange) return 'orange';
  return null;
}
