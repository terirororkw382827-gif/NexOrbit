...
<style>
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color:transparent;}
  ...
  /* Plinko win chip — pops up dead-center on the board when the ball
     lands in a paying slot, then flies upward and fades out. */
...
function setPlinkoBetMax(){ plinkoBetInput.value = Math.min(PLINKO_MAX_BET, Math.max(PLINKO_MIN_BET, Math.floor(Math.min(balance, PLINKO_MAX_BET)))); }
...
function onPlinkoBtn(){
  ...
  if(bet > balance){ plinkoStatusLine.textContent = 'Недостаточно средств'; return; }
  ...
  plinkoStatusLine.textContent = 'Шар летит...';
}
...
function finishPlinkoDrop(state){
  ...
  plinkoStatusLine.textContent = payout > 0
    ? \`Шар попал в \${plinkoFmtMult(mult)}x — выигрыш \${fmt(payout)}\`
    : 'Шар попал в 0x — ставка сгорела';
  if(payout > 0){
    plinkoWinChip.textContent = \`+\${fmt(payout)} @ \${plinkoFmtMult(mult)}x\`;
...
async function loadGameState() {
  ...
  if (s.leftoverPlinkoBets) {
    for (const key of Object.keys(s.leftoverPlinkoBets)) {
      if (users[key]) { users[key].balance += s.leftoverPlinkoBets[key]; refunded = true; }
    }
  }
  if (refunded) {
...
let pendingPlinkoBets = {}; // key -> amount staked, cleared once plinko_result comes back in
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (raw) => {
    ...
    if (msg.type === 'plinko_bet') {
      ...
      user.balance -= amount;
      await saveUsers();
      pendingPlinkoBets[authed] = amount;
      await saveGameState();
      sendTo(ws, { type: 'balance', balance: user.balance });
    }
...
async function refundUnresolvedBetsOnShutdown() {
  ...
  for (const key of Object.keys(pendingPlinkoBets)) {
    if (users[key]) {
      users[key].balance += pendingPlinkoBets[key];
    }
  }
  await saveUsers();
