...
let stairs = {
  ...
  steps:[], ranOut:false, gen:null, playIndex:0, genCumX:0,
...
// unchanged — it still yields one step at a time, generated right before
// the cube needs it. The flight itself now runs on a real Matter.js world:
// each generated step becomes a solid static body (a thick block, not a
// thin platform, so the staircase silhouette has zero gaps and the cube
// physically cannot pass through it), and the cube is a real dynamic body
// with gravity, restitution and friction handled by the engine's own
// collision solver instead of hand-rolled math.
// ---------------------------------------------------------------------------
const { Engine, World, Bodies, Body, Events, Vector } = Matter;

const STAIRS_RESTITUTION = 0.55;   // bounciness on landing (0=dead stop, 1=perfectly elastic)
const STAIRS_FRICTION = 0.12;      // surface friction applied on contact (bleeds horizontal speed)
const STAIRS_SETTLE_SPEED = 18;    // px/step — below this (while touching a step) the cube is "settled"
...
// Real launch kinematics for each hop — genuine initial velocities handed
// to the Matter body. Where it actually lands is decided by the engine's
// real collision solver against the real step bodies, not by scripted
// duration/target math, so a hop can legitimately land short, land long,
// or take several bounces to clear a wide step.
const STAIRS_HOP_VX = 7.5;         // px/step — base horizontal launch speed
const STAIRS_HOP_VY0 = 6.2;        // px/step — base upward launch speed (arcs up, then gravity brings it down)
...
let stairsStepBodies = []; // parallel to stairs.steps, same index
...
function stairsCreateStepBody(step, index, x0, y0, w){
  ...
  const body = Bodies.rectangle(cx, cy, w, STAIRS_BLOCK_DEPTH, {
    isStatic: true,
    friction: STAIRS_FRICTION,
    restitution: STAIRS_RESTITUTION,
    label: 'step',
...
function stairsEnsureGenerated(minIndex, minX){
  while(!stairs.ranOut && (stairs.steps.length <= minIndex || stairs.genCumX < minX)){
    ...
    if(next){
      ...
      stairsStepBodies[i] = stairsCreateStepBody(next, i, x0, 0, w);
    } else {
      stairs.ranOut = true;
    }
  }
  // Drop step bodies that are far behind the cube — keeps the world's
  // body count bounded on long runs instead of growing forever.
  if(stairsCubeBody){
    const behindX = stairsCubeBody.position.x - 2000;
    while(stairsStepBodies.length && stairsStepBodies[0] && stairsStepBodies[0].bounds.max.x < behindX){
      World.remove(stairsWorld, stairsStepBodies[0]);
      stairsStepBodies.shift();
    }
  }
}
...
function startStairsFlight(){
  ...
  stairsEnsureGenerated(STAIRS_RENDER_AHEAD, STAIRS_X_LOOKAHEAD_PX);
...
// Gives the cube a genuine initial launch velocity via the Matter body —
// no target position or duration is baked in. Where it actually lands is
// decided by the engine's own collision solver against the real step
// bodies (see the collisionStart handler above and stairsPhysicsStep below).
function launchTowardsStep(stepIdx){
  ...
  Body.setVelocity(stairsCubeBody, {
    x: STAIRS_HOP_VX * speedMult,
    y: -STAIRS_HOP_VY0 * speedMult // negative = upward, gravity pulls it back down
  });
...
// enough to slip through the solid step blocks. Landing itself is driven
// by the collisionStart listener above (stairsTouchingStepIndex) plus a
// speed check here — once the cube is genuinely resting (touching a step
// and slow enough) for STAIRS_SETTLE_PAUSE ms, the run advances.
let stairsAccMs = 0;
function stairsPhysicsStep(dtMs){
  ...
  while(stairsAccMs >= STAIRS_FIXED_DT){
    ...
    stairsWasTouching = false; // collisionStart re-sets this to true if still in contact
    ...
    // The run is truly over once no more steps can ever be generated
    // (ranOut) AND the cube's real x has gone past the rightmost edge of
    // every step that was ever generated (stairs.genCumX) — at that point
    // it is unambiguously past all solid geometry, whether it landed
    // cleanly on the last step or overshot/bounced past it. Checking
    // "no bodies left" instead (the old, wrong condition) almost never
    // becomes true, since old bodies are only cleaned up once ~2000px
    // behind the cube — so a cube that overshot the last step just kept
    // falling through empty space forever without the run ever ending.
    if(stairs.ranOut && stairsCubeBody.position.x > stairs.genCumX){
      stairs.flightState = 'ranout'; finishStairsRun(); return;
    }
    if(stairsTouchingStepIndex !== null){
      const speed = Vector.magnitude(stairsCubeBody.velocity);
      if(stairsWasTouching && speed < STAIRS_SETTLE_SPEED){
        if(stairs.flightState !== 'settled'){
          stairs.flightState = 'settled';
          stairs.landedIndex = stairsTouchingStepIndex;
          stairs.settleStartTs = performance.now();
        }
      } else if(stairsWasTouching){
        stairs.flightState = 'bounce';
      }
    }
  }
  if(stairs.flightState === 'settled' && performance.now() - stairs.settleStartTs >= STAIRS_SETTLE_PAUSE){
    onStepLanded();
  }
}
...
function drawStairsCanvas(){
  ...
  const refIdx = (stairsTouchingStepIndex != null) ? stairsTouchingStepIndex : stairs.playIndex;
