...
function plinkoBuildBoard(){
  ...
  const slotAreaH = Math.max(34, h*0.11);
  const topY = h*0.10;
  const bottomOfPegsY = h - slotAreaH - h*0.03;
  const rowGapY = (bottomOfPegsY - topY) / (PLINKO_ROWS - 1);

  const pegR = Math.max(2.5, slotW*0.11);
  const ballR = Math.max(5, slotW*0.30);
  ...
  // Divider walls: one at every slot boundary (14 total — 12 internal +
  // 2 outer), each a thin tall rectangle running from the peg field down
  // through the bottom of the slot area, so the ball is physically
  // channeled straight down into a slot once it clears the last peg row.
  const wallTopY = bottomOfPegsY - rowGapY*0.4;
  const wallBotY = h - h*0.01;
  const wallH = wallBotY - wallTopY;
  const wallOpts = { isStatic:true, restitution:0.3, friction:0.05, label:'plinko_wall' };
  for(let i=0; i<=PLINKO_SLOTS; i++){
    const x = marginX + i*slotW;
    const body = Bodies.rectangle(x, wallTopY + wallH/2, Math.max(2, slotW*0.05), wallH, wallOpts);
...
function finishPlinkoDrop(){
  ...
  plinkoStatusLine.textContent = payout > 0
    ? \`Шар попал в \${fmt(mult)}x — выигрыш \${fmt(payout)}\`
...
function drawPlinkoCanvas(){
  ...
  for(let i=0; i<PLINKO_SLOTS; i++){
    ...
    plinkoCtx.font = \`700 \${Math.max(9, b.slotW*0.26)}px Consolas, monospace\`;
    plinkoCtx.textAlign = 'center';
    plinkoCtx.textBaseline = 'middle';
    plinkoCtx.fillText(fmt(PLINKO_MULTS[i])+'x', x0+b.slotW/2, b.wallTopY + ((h-h*0.01)-b.wallTopY)/2);
  }
