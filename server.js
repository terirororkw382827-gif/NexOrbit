...
const STAIRS_COLOR_CHANCES = { rainbow: 0.005, diamond: 0.05, gold: 0.10, black: 0.025 }; // halved from the original 1/10/20/5%; sums to 0.18, remaining 0.82 is a normal step (or a wide complication/bonus step)
const STAIRS_COLOR_BONUS = { gold: 0.05, diamond: 0.10, rainbow: 0.50, black: -0.01 }; // total multiplier contribution of that step (replaces the usual +0.01)
const STAIRS_COLOR_LABELS = {
  gold: 'Золотая ступень +0.05', diamond: 'Алмазная ступень +0.10',
  rainbow: 'Радужная ступень +0.50', black: 'Чёрная ступень взорвалась! -0.01'
};
...
function stairsApplyStepEffect(step){
  if(step.color){
    ...
    // contribution matches STAIRS_COLOR_BONUS exactly (0.05/0.10/0.50/-0.01).
    stairs.colorBonusAccum += STAIRS_COLOR_BONUS[step.color] - 0.01;
    if(step.color === 'black'){
      // the step "explodes": a strong forward+upward kick, same idea as a
      // real explosive impulse launching the cube onward
      const v = stairsCubeBody.velocity;
      Body.setVelocity(stairsCubeBody, { x: v.x + 30, y: Math.min(v.y, 0) - 16 });
    }
  }
