...
const STAIRS_GAP_CHANCE = 0.003;   // 0.3% — this step has NO floor at all (a real hole in the
                                    // geometry); falling into it while over its span is instant death
const STAIRS_WALL_CHANCE = 0.003;  // 0.3% — a solid wall at this step's far edge, shorter than the
...
const STAIRS_COLOR_CHANCES = { rainbow: 0.005, diamond: 0.05, gold: 0.10, black: 0.01 }; // black lowered to 1%; sums to 0.165, remaining 0.835 is a normal step (or a wide complication/bonus step)
const STAIRS_COLOR_BONUS = { gold: 0.05, diamond: 0.10, rainbow: 0.50, black: -0.10 }; // total multiplier contribution of that step (replaces the usual +0.01)
...
function stairsApplyStepEffect(step){
  if(step.color){
    ...
    // The base ramp (step.multiplier = idx/100) already credits this step's
    // ordinary +0.01; we add just the extra on top so the step's TOTAL
    // contribution matches STAIRS_COLOR_BONUS exactly (0.05/0.10/0.50/-0.10).
