...
function plinkoBuildBoard(){
  ...
  plinkoWallBodies.push(
    Bodies.rectangle(marginX, outerWallH/2, Math.max(3, slotW*0.06), outerWallH, outerWallOpts),
    Bodies.rectangle(marginX+boardW, outerWallH/2, Math.max(3, slotW*0.06), outerWallH, outerWallOpts)
  );
  ...
  for(let i=1; i<PLINKO_SLOTS; i++){
    ...
    const body = Bodies.rectangle(x, wallTopY + wallH/2, Math.max(2, slotW*0.05), wallH, dividerOpts);
...
function onPlinkoBtn(){
  ...
  plinkoBall = Bodies.circle(plinkoBoard.centerX + jitter, plinkoBoard.topY - plinkoBoard.rowGapY*0.8, plinkoBoard.ballR, {
    restitution: 0.55, friction: 0.05, frictionAir: 0.0005, label:'plinko_ball'
  });
