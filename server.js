...
function plinkoBuildBoard(){
  ...
  // Tight side margins — just enough room for the outer wall itself —
  // so the peg triangle's base actually reaches close to the edges of
  // the board instead of looking like it's floating in the middle of it.
  const marginX = Math.max(6, w*0.012);
  const boardW = w - marginX*2;
  const slotW = boardW / PLINKO_SLOTS;
  const centerX = marginX + boardW/2;

  // A short, compact single-line strip for the multiplier labels —
  // was way taller than it needed to be before.
  const slotAreaH = Math.max(22, h*0.045);
  const topY = h*0.08;
  ...
  //
  // Thickness is deliberately generous (0.22*slotW, was 0.06) — at the old
  // thickness a fast ball could cross the entire wall within one physics
  // step and tunnel straight through without Matter ever registering an
  // overlap (Matter only checks for overlap at discrete steps, it doesn't
  // sweep the path in between). Thick geometry is the standard fix for
  // that, since there's no built-in continuous collision detection here.
  const wallBotY = h - h*0.01;
  const outerWallOpts = { isStatic:true, restitution:0, friction:0.05, label:'plinko_wall' };
  const outerWallH = wallBotY - 0;
  const outerWallThickness = Math.max(8, slotW*0.22);
  plinkoWallBodies.push(
    Bodies.rectangle(marginX, outerWallH/2, outerWallThickness, outerWallH, outerWallOpts),
    Bodies.rectangle(marginX+boardW, outerWallH/2, outerWallThickness, outerWallH, outerWallOpts)
  );

  // Roof — closes the top of the board so the ball can never fly out
  // above the peg triangle. It behaves as an active bumper on contact
  // (label:'plinko_bumper' — same PLINKO_BUMPER_KICK / plinkoMarkBumperHit
  // logic below, so it genuinely kicks the ball back down instead of just
  // passively bouncing it), but it's pushed into plinkoWallBodies so the
  // wall-drawing loop below picks it up and paints it the exact same
  // translucent white as the side walls — not the black/yellow bumper
  // style the (now removed) side bumpers used.
  //
  // Earlier version clamped the roof's thickness down to a 4px MINIMUM
  // so it would still be visible on short boards — but on a compact
  // phone-sized canvas there can be less than 4px of room above the
  // ball's spawn point at all, so that floor could still force a real
  // (if visually tiny/imperceptible) overlap with the ball. That's
  // exactly what caused the roof to still repel the ball even though it
  // looked untouched: a couple of overlapping pixels are invisible at
  // this scale but very much real to Matter's collision detection.
  //
  // Fix: keep the SAME thickness as the side walls always (never
  // shrink it), and instead let the body's position go slightly ABOVE
  // y=0 when there isn't enough room — the canvas naturally clips
  // anything drawn above its own top edge, so only however much of the
  // roof actually fits stays visible, while its bottom edge (the part
  // that matters for collision) is guaranteed to sit GAP_PX above the
  // ball's spawn point on every screen size, no exceptions.
  const spawnTopEdgeY = (topY - rowGapY*0.8) - ballR;
  const ROOF_GAP_PX = 3; // real, guaranteed clearance between roof and ball at spawn
  const roofThickness = outerWallThickness;
  const roofY = spawnTopEdgeY - ROOF_GAP_PX - roofThickness/2;
  plinkoWallBodies.push(
    Bodies.rectangle(centerX, roofY, boardW, roofThickness, {
