...
function plinkoBuildBoard(){
  ...
  // The ball's spawn point (topY - rowGapY*0.8, see onPlinkoBtn) is left
  // completely untouched, which only leaves a small, fixed sliver of
  // room above it — so instead of using the same fat thickness as the
  // side walls (which wouldn't fit there without overlapping the ball),
  // the roof's thickness is capped to whatever headroom is actually
  // available, with a real GAP_PX gap kept clear before the ball's top
  // edge. It just ends up visually thinner than the side walls on short
  // boards, same color either way.
  const spawnTopEdgeY = (topY - rowGapY*0.8) - ballR;
  const ROOF_GAP_PX = 4;   // real clearance always kept between roof and ball at spawn
  const ROOF_TOP_MARGIN = 2; // keeps the roof's own top edge from clipping past y=0
  const roofMaxThickness = spawnTopEdgeY - ROOF_GAP_PX - ROOF_TOP_MARGIN;
  const roofThickness = Math.max(4, Math.min(outerWallThickness, roofMaxThickness));
  const roofY = ROOF_TOP_MARGIN + roofThickness/2;
  plinkoWallBodies.push(
