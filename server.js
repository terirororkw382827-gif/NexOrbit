...
function pickBotTargetMult() {
  // Mostly 1.1x-2x (median ≈1.4x, ~90% under ~2.3x), with a thin tail
  // beyond that for the occasional bigger target — "bots mostly cash out
  // between 1x and 2x".
  const r = Math.random();
  const target = 1.05 + (-Math.log(1 - r)) * 0.55;
  return Math.round(Math.min(30, target) * 100) / 100;
}
...
function spawnBotBets() {
  ...
  for (const name of BOT_NAMES) {
    ...
    const target = pickBotTargetMult();
    // Same fairness formula the real crashPoint itself is built on
    // (P(reach target) = (1-edge)/target), just rolled privately per bot
    // with the bot's own edge instead of shared with the round.
    const winProb = Math.min(1, (1 - BOT_HOUSE_EDGE) / target);
    scheduledBots.push({
      name, amount, target, botWillWin: Math.random() < winProb,
...
function admitDueBots() {
  ...
  for (let i = scheduledBots.length - 1; i >= 0; i--) {
    ...
    bets['bot:' + sb.name] = {
      ...
      isBot: true, botTarget: sb.target, botWillWin: sb.botWillWin
    };
...
function resolveBotCashouts() {
  for (const key of Object.keys(bets)) {
    const b = bets[key];
    // A bot that rolled "lose" just never cashes — it busts naturally along
    // with everyone else still active when doCrash() resolves the round.
    // A bot that rolled "win" cashes once the real multiplier display
    // actually reaches its target; if this particular round's rocket
    // crashes before ever reaching it, the bot has to bust too — there's no
    // way to visibly cash out at a multiplier the display never showed.
    // That's an unavoidable small compromise for staying independent of
    // the shared crashPoint rather than reading from it directly.
    if (b.isBot && b.status === 'active' && b.botWillWin && currentMultiplier >= b.botTarget) {
      b.status = 'cashed';
      b.mult = b.botTarget;
    }
  }
...
function doCrash() {
  ...
  Object.values(bets).forEach(b => {
    ...
    if (b.isBot && b.botWillWin) {
      // This bot's own independent roll said it should win — that roll
      // already has BOT_HOUSE_EDGE priced into it and is meant to be the
      // ONLY thing deciding its fate. Busting it here just because this
      // particular round's shared crashPoint happened to land below its
      // target silently stacked a second house edge on top (effectively
      // (1-edge)/target chained twice), so bots came out losing noticeably
      // more than the stated 1.5%. Honoring the roll and cashing it out at
      // the real ceiling (crashPoint) instead of its original target keeps
      // bots genuinely independent of the round, at the cost of an
      // occasional bot "cashing out" right as the rocket crashes — a small
      // cosmetic compromise, not a fairness one.
      b.status = 'cashed';
      b.mult = crashPoint;
    } else {
      b.status = 'busted';
