...
function handleServerMessage(msg){
  ...
  if(msg.type === 'plinko_outcome'){
    balance = msg.balance; updateBalanceDisplay();
    const pending = plinkoPendingDrops[msg.dropId];
    if(pending){
      delete plinkoPendingDrops[msg.dropId];
      plinkoSpawnBall(pending.bet, msg.flips, msg.slot, msg.mult, msg.payout);
    }
    return;
  }
...
// =====================================================================
// PLINKO
// The OUTCOME is decided entirely by the server the instant the bet is
// placed — PLINKO_ROWS independent fair 50/50 coin flips, exact
// binomial(PLINKO_ROWS,0.5) / Pascal's triangle (see plinkoGenerateOutcome
// server-side). The Matter.js board below is now PURELY a visual
// animation: the ball is steered along a path precomputed from those same
// flips (see plinkoSpawnBall/plinkoPhysicsStep), so no amount of
// restitution/friction tuning or peg-channeling can ever change the
// actual probability of any slot — those numbers are exact and known in
// advance, which is the whole point of moving off "physics decides".
//
// Multiplier layout is deliberately the OPPOSITE of what it might look
// like at first glance: 0x sits alone in the CENTER, not the edges.
// Binomial(10,0.5) is heavily weighted toward the middle column, so the
// center is the MOST likely landing spot and the edges the LEAST likely —
// the payout table mirrors that (low in the middle, high at the edges),
// same as every real Plinko board (arcade or casino) for the same reason.
// =====================================================================
const PLINKO_MIN_BET = 1;
const PLINKO_MAX_BET = 1000;
const PLINKO_ROWS = 10;                 // must match the server's copy exactly — layout only, the
const PLINKO_SLOTS = PLINKO_ROWS + 1;   // server's PLINKO_ROWS is what actually decides the outcome
// NOTE on RTP: with true fair 50/50 flips, slot k's probability is exactly
// C(PLINKO_ROWS,k)/2^PLINKO_ROWS — e.g. slot 5 (the 0x center) hits 24.6%
// of the time, slots 4/6 (1.5x) hit 20.5% EACH. Weighting the current
// table by those exact probabilities gives RTP ≈ 146.5% — a real, guaranteed
// loss for the house on average, not an estimate. Flagged to fix separately
// from this architecture change; the table itself is unchanged for now.
const PLINKO_MULTS = [10,5,3,2,1.5,0,1.5,2,3,5,10];
const PLINKO_SLOT_COLORS = ['#ffd23f','#ff9f1c','#ff6b35','#ff3d5a','#55506a','#3a3a45','#55506a','#ff3d5a','#ff6b35','#ff9f1c','#ffd23f'];
...
// One entry per ball currently in flight — { body, bet, flips, slot, mult,
// payout, pathX, settled, settleStartTs }. mult/payout/slot are already
// known (the server decided them before this ball ever spawns) — settled
// is purely about WHEN to tidy up and show the result, never about WHAT
// the result is.
let plinkoBalls = [];
let plinkoAccMs = 0;
let plinkoLastTs = 0;
// dropId -> {bet}, waiting on the server's plinko_outcome reply before the
// actual ball body gets created — see plinkoDropOne / plinkoSpawnBall.
let plinkoDropSeq = 0;
let plinkoPendingDrops = {};

// Bumper-kick mechanism removed — the roof used to be labeled
// 'plinko_bumper' and get an artificial energy kick on contact, which was
// a source of bounce. The roof is now a plain zero-restitution wall (see
// plinkoBuildBoard above), so nothing on the board actively re-energizes
// the ball anymore; gravity and friction are the only forces after a hit.
...
// Called once the server's plinko_outcome for this drop arrives — the
// slot/mult/payout are already final at this point; everything below is
// pure animation.
function plinkoSpawnBall(bet, flips, slot, mult, payout){
  // Strictly centered drop, with a tiny random jitter so repeated drops
  // don't all take the exact same path visually.
  ...
  // Precompute the ball's intended x at every row from the SAME flips the
  // server used to pick the slot — step size is slotW/2 because adjacent
  // peg rows are offset by half a slot (see plinkoBuildBoard), which is
  // exactly what lands this at the target slot's center after all flips:
  // starting at centerX and taking (rights - lefts)*slotW/2 nets out to
  // (slot - PLINKO_SLOTS/2)*slotW, matching that slot's real position.
  const pathX = [plinkoBoard.centerX + jitter];
  for(let i=0;i<flips.length;i++){
    pathX.push(pathX[i] + (flips[i] ? 1 : -1) * plinkoBoard.slotW/2);
  }

  const state = { body, bet, flips, slot, mult, payout, pathX, settled:false, settleStartTs:0 };
...
function plinkoPhysicsStep(dtMs){
  ...
  while(plinkoAccMs >= PLINKO_FIXED_DT){
    ...
    // Steer every ball toward its precomputed path BEFORE the engine
    // resolves this step's collisions — a gentle continuous position
    // correction (not a hard snap), so peg collisions still visibly nudge
    // the ball moment-to-moment, but it's mathematically guaranteed to
    // converge on the RNG-decided slot by the time it reaches the bottom
    // regardless of how any individual peg graze happens to deflect it.
    for(const s of plinkoBalls){
      const b = plinkoBoard;
      const rowF = Math.max(0, Math.min(PLINKO_ROWS, (s.body.position.y - b.topY) / b.rowGapY));
      const r0 = Math.floor(rowF), r1 = Math.min(PLINKO_ROWS, r0+1);
      const frac = rowF - r0;
      const targetX = s.pathX[r0] + (s.pathX[r1]-s.pathX[r0])*frac;
      const newX = s.body.position.x + (targetX - s.body.position.x) * 0.08;
      Body.setPosition(s.body, { x: newX, y: s.body.position.y });
    }

    Engine.update(plinkoEngine, PLINKO_FIXED_DT);

    // Settled is purely a "has it visually stopped moving near the
    // bottom" check now — it decides WHEN to wrap up, never WHAT the
    // result is (that was already decided server-side before this ball
    // even spawned).
    for(const s of plinkoBalls){
      const nearBottom = s.body.position.y >= plinkoBoard.wallTopY;
      const speed = Vector.magnitude(s.body.velocity);
      if(nearBottom && speed < PLINKO_SETTLE_SPEED){
...
function finishPlinkoDrop(state){
  ...
  // mult/payout are already final — the server decided them at bet time,
  // nothing left to read off the board and nothing left to tell the
  // server (no more plinko_result round-trip).
  const { mult, payout } = state;
...
// ---------------------------------------------------------------------------
// PLINKO — the outcome is a real RNG draw (PLINKO_ROWS independent fair
// 50/50 flips = exact binomial(PLINKO_ROWS, 0.5) / Pascal's triangle),
// decided HERE the instant the bet is placed. The client's Matter.js board
// is purely a visual animation of a path that's mathematically guaranteed
// to end at this outcome — it never has any say in the result. Keep
// PLINKO_ROWS/PLINKO_MULTS here in sync with the client copy (same names,
// inside CLIENT_HTML) if either ever changes — the client only uses its
// copy for layout/rendering, never to decide a payout.
const PLINKO_ROWS = 10;   // -> 11 slots (PLINKO_ROWS+1), indices 0..10
const PLINKO_MULTS = [10,5,3,2,1.5,0,1.5,2,3,5,10];

function plinkoGenerateOutcome() {
  const flips = [];
  let slot = 0;
  for (let i = 0; i < PLINKO_ROWS; i++) {
    const right = Math.random() < 0.5;
    flips.push(right ? 1 : 0);
    if (right) slot++;
  }
  return { flips, slot, mult: PLINKO_MULTS[slot] };
}
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (raw) => {
    ...
    // ----- PLINKO -----
    // The server now decides the outcome itself — PLINKO_ROWS independent
    // fair coin flips (plinkoGenerateOutcome, true binomial distribution) —
    // the instant the bet is placed, and credits the payout in the same
    // breath. The client's board/ball is purely a visual animation of a
    // path guaranteed to end at whatever slot is sent back here; it's
    // never trusted to report a result anymore, so there's no more
    // separate plinko_result message, and no window where money is
    // "staked but unresolved" the way pendingStairsBets still is for
    // Stairs — the bet is fully resolved before this handler even returns.
    if (msg.type === 'plinko_bet') {
      const amount = Math.floor(Number(msg.amount) * 100) / 100;
      if (!amount || amount < 1 || amount > 1000 || amount > user.balance) {
        return sendTo(ws, { type: 'error', error: 'Некорректная сумма ставки' });
      }
      user.balance -= amount;
      const { flips, slot, mult } = plinkoGenerateOutcome();
      const payout = Math.round(amount * mult * 100) / 100;
      user.balance += payout;
      await saveUsers();
      await saveGameState();
      sendTo(ws, { type: 'plinko_outcome', dropId: msg.dropId, flips, slot, mult, payout, balance: user.balance });
    }
