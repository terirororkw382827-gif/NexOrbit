...
<style>
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color:transparent;}
  ...
  .aim-arrow{position:absolute;left:20%;top:38%;font-size:30px;color:#fff;transform-origin:0% 50%;
...
<div class="app" id="app">
  ...
  <div class="view" id="viewStairs">
    ...
    <div class="stairs-stage" id="stairsStage">
      ...
      <div class="aim-arrow" id="aimArrow" style="display:none;">➤</div>
...
const STAIRS_BASE_STEP_COST = 0.055;   // energy spent clearing one normal step
const STAIRS_MAX_ENERGY = 3.2;         // energy from a perfect throw (angle=0°/flat, power=center)
const STAIRS_TIER_SIZE = 50;           // difficulty ramps up every N steps
const STAIRS_TIER_COST_GROWTH = 0.15;  // +15% step cost per tier
const STAIRS_TIER_EFFECT_GROWTH = 0.15;// complications get +15%/tier stronger, bonuses get weaker by the same factor
const STAIRS_TIER_WIDTH_GROWTH = 0.20; // ALL steps get +20% longer per tier (every 50 steps)
...
let stairs = {
  ...
  steps:[], ranOut:false, gen:null, playIndex:0,
  angleStartTs:0, powerStartTs:0,
  flightState:null, x:0, y:0, vx:0, vy:0,
  landedIndex:null, flightOverIndex:null,
...
// ---------------------------------------------------------------------------
// PHYSICS + LAZY STEP GENERATION
// The odds/effects math (cost per step, tiers, complications/bonuses) is
// unchanged from before — it just yields one step at a time now, generated
// right before the cube needs it, instead of precomputing the whole run.
// The flight itself is a real integrated simulation: gravity accelerates
// the cube every frame, landings apply restitution (bounce) and friction
// (horizontal speed loss), and the camera lerps toward the cube's actual
// position instead of snapping between fixed per-step anchors.
// ---------------------------------------------------------------------------
const STAIRS_GRAVITY = 2200;       // px/s^2
const STAIRS_RESTITUTION = 0.38;   // vertical speed kept after each bounce
const STAIRS_FRICTION = 0.55;      // horizontal speed kept after each bounce
const STAIRS_BOUNCE_MIN_VEL = 40;  // px/s — below this the cube is considered settled
const STAIRS_SETTLE_PAUSE = 90;    // ms standing on a step before the next throw
const STAIRS_RENDER_AHEAD = 9;     // how many upcoming steps to keep generated/drawn
const STAIRS_CAM_LERP = 0.14;      // camera smoothing (per-frame-at-60fps factor)
// Real launch kinematics for each hop. Unlike the old back-solved version
// (which computed a fake vy that was GUARANTEED to land exactly on the
// intended step after a fixed duration, no matter what), these are genuine
// initial velocities: gravity is integrated every frame and a real
// per-frame collision check against the actual step geometry (see
// stairsCurrentStepIndexAt) decides where and when the cube actually lands.
// A hop can now legitimately land short, land long (skipping a step), or
// take several bounces to clear a wide step — because it's simulated, not
// scripted.
const STAIRS_HOP_VX = 300;         // px/s — base horizontal launch speed
const STAIRS_HOP_VY0 = 260;        // px/s — base upward launch speed (arcs up, then gravity brings it down)

function newStairsGen(energy){
  return {
    energy, cubeWeight:1, cubeSpeed:1, cubeSize:1,
    longRemain:0, longMult:1, shortRemain:0, shortMult:1,
    springyRemain:0, slowRemain:0, idx:0
  };
}

// Generates exactly one more step from the run's current energy/effect
// state, or returns null the moment the cube can't afford the next one.
function stairsGenNext(g){
  g.idx++;
  const idx = g.idx;
  const tier = Math.floor((idx-1) / STAIRS_TIER_SIZE);
  const tierCostMult = 1 + tier*STAIRS_TIER_COST_GROWTH;
  const tierWidthMult = 1 + tier*STAIRS_TIER_WIDTH_GROWTH; // ALL steps get longer every 50 steps

  let widthMult = tierWidthMult;
  if(g.longRemain > 0){ widthMult *= g.longMult; g.longRemain--; }
  if(g.shortRemain > 0){ widthMult *= g.shortMult; g.shortRemain--; }

  let cost = STAIRS_BASE_STEP_COST * widthMult * g.cubeWeight * tierCostMult;
  const springyNow = g.springyRemain > 0;
  if(springyNow){ cost *= 0.85; g.springyRemain--; }
  cost = cost / g.cubeSpeed;

  if(g.energy < cost) return null; // not enough energy left — run ends here
  g.energy -= cost;

  const slowNow = g.slowRemain > 0;
  if(slowNow) g.slowRemain--;
  ...
  if(isWide){
    ...
    if(Math.random() < 0.65){
      ...
      if(effect === 'shrink') g.cubeSize = Math.max(0.4, g.cubeSize*0.75);
      else if(effect === 'lengthen25'){ g.longRemain = 25; g.longMult = 1 + 0.5*tierEffMult; }
      else if(effect === 'heavy'){ g.cubeWeight *= 1 + 0.5*tierEffMult; }
      else if(effect === 'slow'){ g.slowRemain = 15; }
    } else {
      special = 'green';
      effect = BONUSES[Math.floor(Math.random()*BONUSES.length)];
      if(effect === 'grow') g.cubeSize = Math.min(2.2, g.cubeSize*1.25);
      else if(effect === 'shorten10'){ g.shortRemain = 10; g.shortMult = Math.max(0.15, 1 - 0.5/tierEffMult); }
      else if(effect === 'speed'){ g.cubeSpeed *= 1 + 0.5/tierEffMult; }
      else if(effect === 'springy'){ g.springyRemain = 14; }
    }
  }
  ...
  return {
    ...
    cubeSize: g.cubeSize, slow: slowNow, springy: springyNow
  };
}

// Tops up stairs.steps so at least uptoIndex entries exist, pulling
// fresh ones from the generator on demand. Flips stairs.ranOut the moment
// the generator can no longer afford the next step.
function stairsEnsureGenerated(uptoIndex){
  while(!stairs.ranOut && stairs.steps.length <= uptoIndex){
    const next = stairsGenNext(stairs.gen);
    if(next) stairs.steps.push(next);
    else stairs.ranOut = true;
  }
}
...
function startStairsFlight(){
  ...
  // cos(angle) gives that falloff smoothly, physics-style, same as the
  // horizontal component of a launch velocity.
  const angleQuality = Math.cos(stairs.angle * Math.PI/180);
  const powerQuality = Math.max(0, 1 - Math.abs(stairs.power - 50)/50);
  const energy = angleQuality * powerQuality * STAIRS_MAX_ENERGY;

  stairs.gen = newStairsGen(energy);
  stairs.steps = [];
  stairs.ranOut = false;
  stairsEnsureGenerated(STAIRS_RENDER_AHEAD);
  ...
  if(stairs.steps.length === 0){
    stairsStatusLine.textContent = 'Куб даже не сдвинулся с места...';
    setTimeout(finishStairsRun, 400);
  } else {
...
// Gives the cube a genuine initial launch velocity — no target position or
// duration is baked in. Where it actually lands is decided every frame by
// stairsPhysicsStep's real collision check against the true step geometry.
function launchTowardsStep(stepIdx){
  const step = stairs.steps[stepIdx] || stairs.steps[stairs.steps.length-1];
  const speedMult = (step.slow ? 0.65 : 1) * (step.springy ? 1.2 : 1);
  stairs.vx = STAIRS_HOP_VX * speedMult;
  stairs.vy = -STAIRS_HOP_VY0 * speedMult; // negative = upward, gravity pulls it back down
  stairs.flightState = 'air';
  stairs.landedIndex = null;
}

// Real collision detection: walks forward from the last step the cube
// actually landed on to find which generated step's horizontal span the
// cube's current x sits over (steps are contiguous, so this is a simple
// forward scan — the cube never needs to look backwards). Every step drops
// the same STAIRS_STEP_H, so that step's top surface is just (index+1)*H.
function stairsCurrentStepIndexAt(x){
  const fromIdx = stairs.playIndex;
  const base = stairsCumulativeXY(stairs.steps, fromIdx);
  let acc = base.x;
  for(let i=fromIdx; i<stairs.steps.length; i++){
    const w = STAIRS_STEP_W * stairs.steps[i].widthMult;
    if(x < acc + w) return { index:i, xStart:acc, xEnd:acc+w, surfaceY:(i+1)*STAIRS_STEP_H };
    acc += w;
  }
  return null; // not generated far enough ahead yet (or genuinely ran out)
}

// One physics tick (dt in seconds): integrates real gravity every frame,
// then checks real collision against whichever step's rectangle the cube
// is actually over. On contact it applies restitution (bounce) + friction,
// decaying bounces until the cube settles — same treatment as a real
// bouncing ball, and it can never end up rendered "inside" a step because
// the surface it collides with is always the true geometry under its
// current x, not a pre-picked target.
function stairsPhysicsStep(dt){
  if(stairs.flightState === 'air' || stairs.flightState === 'bounce'){
    stairs.vy += STAIRS_GRAVITY * dt;
    stairs.x += stairs.vx * dt;
    stairs.y += stairs.vy * dt;

    // keep enough steps generated ahead of wherever the cube has actually flown to
    stairsEnsureGenerated(stairs.playIndex + STAIRS_RENDER_AHEAD);

    const over = stairsCurrentStepIndexAt(stairs.x);
    if(!over){
      // Genuinely ran out of budget mid-flight — no further step exists to land on.
      if(stairs.ranOut){ stairs.flightState = 'ranout'; finishStairsRun(); }
      return;
    }
    stairs.flightOverIndex = over.index;

    if(stairs.y >= over.surfaceY){
      stairs.y = over.surfaceY;
      if(Math.abs(stairs.vy) > STAIRS_BOUNCE_MIN_VEL){
        stairs.vy = -stairs.vy * STAIRS_RESTITUTION; // real bounce off the real surface
        stairs.vx *= STAIRS_FRICTION;                // friction bleeds speed each contact
        stairs.flightState = 'bounce';
      } else {
        stairs.vy = 0; stairs.vx = 0;
        stairs.flightState = 'settled';
        stairs.landedIndex = over.index; // whichever step it genuinely came to rest on
        stairs.settleStartTs = performance.now();
      }
    }
  } else if(stairs.flightState === 'settled'){
    if(performance.now() - stairs.settleStartTs >= STAIRS_SETTLE_PAUSE){
      onStepLanded();
    }
  }
}
...
function onStepLanded(){
  // Sync to the step the cube ACTUALLY reached — real collision can land it
  // short, or carry it past one or more steps in a single long bounce.
  stairs.playIndex = stairs.landedIndex;
  ...
  stairs.playIndex++;
  stairsEnsureGenerated(stairs.playIndex + STAIRS_RENDER_AHEAD);
  if(stairs.playIndex < stairs.steps.length){
    launchTowardsStep(stairs.playIndex);
  } else {
    finishStairsRun();
  }
}
...
function drawStairsCanvas(){
  ...
  const refIdx = (stairs.flightOverIndex != null) ? stairs.flightOverIndex : stairs.playIndex;
  const step = stairs.steps[Math.min(refIdx, stairs.steps.length-1)];
  const size = 22 * (step.cubeSize || 1);
  const cx = stairs.x - stairs.camX;
  const cy = stairs.y - stairs.camY - size/2;
  stairsCtx.save();
  stairsCtx.translate(cx, cy);
  // spin rate tied to real distance traveled, so it reads as rolling
  stairsCtx.rotate(((stairs.x*0.06) % 360) * Math.PI/180);
