...
const STAIRS_STEP_ANIM_MS = 200;       // ms per step during playback
...
let stairs = { phase:'idle', bet:0, angle:0, power:0, steps:[], playIndex:0, angleStartTs:0, powerStartTs:0, stepStartTs:0 };
...
// Precomputes the whole run as a list of cleared steps (deterministic
// playback afterwards) — simpler and safer to get right than live physics.
function simulateStairsRun(energy){
  let cubeWeight=1, cubeSpeed=1, cubeSize=1;
  let longRemain=0, longMult=1, shortRemain=0, shortMult=1;
  let springyRemain=0, slowRemain=0;
  const steps = [];
  let idx = 0;
  while(energy > 0 && idx < 3000){
    idx++;
    ...
    if(longRemain > 0){ widthMult *= longMult; longRemain--; }
    if(shortRemain > 0){ widthMult *= shortMult; shortRemain--; }

    let cost = STAIRS_BASE_STEP_COST * widthMult * cubeWeight * tierCostMult;
    const springyNow = springyRemain > 0;
    if(springyNow){ cost *= 0.85; springyRemain--; }
    cost = cost / cubeSpeed;

    if(energy < cost) break; // cube doesn't have enough energy to clear this step — run ends
    energy -= cost;

    const slowNow = slowRemain > 0;
    if(slowNow) slowRemain--;
    ...
    if(isWide){
      ...
      if(Math.random() < 0.65){
        ...
        if(effect === 'shrink') cubeSize = Math.max(0.4, cubeSize*0.75);
        else if(effect === 'lengthen25'){ longRemain = 25; longMult = 1 + 0.5*tierEffMult; }
        else if(effect === 'heavy'){ cubeWeight *= 1 + 0.5*tierEffMult; }
        else if(effect === 'slow'){ slowRemain = 15; }
      } else {
        special = 'green';
        effect = BONUSES[Math.floor(Math.random()*BONUSES.length)];
        if(effect === 'grow') cubeSize = Math.min(2.2, cubeSize*1.25);
        else if(effect === 'shorten10'){ shortRemain = 10; shortMult = Math.max(0.15, 1 - 0.5/tierEffMult); }
        else if(effect === 'speed'){ cubeSpeed *= 1 + 0.5/tierEffMult; }
        else if(effect === 'springy'){ springyRemain = 14; }
      }
    }
    ...
    steps.push({
      ...
      cubeSize, slow: slowNow, springy: springyNow
    });
  }
  return steps;
}
...
function onStairsBtn(){
  if(stairs.phase === 'idle'){
    ...
    stairs = { phase:'aim-angle', bet, angle:0, power:0, steps:[], playIndex:0, angleStartTs: performance.now(), powerStartTs:0, stepStartTs:0 };
...
function startStairsFlight(){
  ...
  stairs.steps = simulateStairsRun(energy);
  stairs.playIndex = 0;
  stairs.phase = 'flying';
  stairs.stepStartTs = performance.now();
  stairsBtn.disabled = true;
  stairsBtn.className = 'main-btn locked';
  stairsBtn.textContent = 'ПОЛЁТ...';
  stairsStatusLine.textContent = stairs.steps.length ? 'Куб летит...' : 'Куб даже не сдвинулся с места...';
  if(stairs.steps.length === 0) setTimeout(finishStairsRun, 400);
}

function finishStairsRun(){
  const finalMult = stairs.steps.length ? stairs.steps[stairs.steps.length-1].multiplier : 0;
...
function drawStairsCanvas(){
  ...
  const idx = Math.min(stairs.playIndex, steps.length-1);

  const startIdx = Math.max(0, idx-2);
  const base = stairsCumulativeXY(steps, startIdx);
  const camPivotX = w*0.30, camPivotY = h*0.30;
  const camOffsetX = camPivotX - base.x, camOffsetY = camPivotY - base.y;

  let dx = base.x, dy = base.y;
  for(let i=startIdx; i<Math.min(steps.length, startIdx+9); i++){
    const st = steps[i];
    const sw = STAIRS_STEP_W*st.widthMult, sh = STAIRS_STEP_H;
    const px = dx + camOffsetX, py = dy + camOffsetY;
    let color = '#16204a';
  ...
  const cur = stairsCumulativeXY(steps, idx);
  const curW = STAIRS_STEP_W*steps[idx].widthMult;
  const landX = cur.x + curW*0.5, landY = cur.y;
  let prevX = landX, prevY = landY;
  if(idx > 0){
    const prev = stairsCumulativeXY(steps, idx-1);
    const prevW = STAIRS_STEP_W*steps[idx-1].widthMult;
    prevX = prev.x + prevW*0.5; prevY = prev.y;
  }
  const dur = STAIRS_STEP_ANIM_MS * (steps[idx].slow ? 1.6 : 1);
  const tFrac = Math.min(1, (performance.now() - stairs.stepStartTs)/dur);
  const cx = (prevX + (landX-prevX)*tFrac) + camOffsetX;
  const cy = (prevY + (landY-prevY)*tFrac) + camOffsetY - 14;

  const size = 22 * (steps[idx].cubeSize || 1);
  stairsCtx.save();
  stairsCtx.translate(cx, cy);
  stairsCtx.rotate(((idx*20) % 360) * Math.PI/180);
...
function stairsFrame(ts){
  if(stairs.phase === 'aim-angle'){
    ...
    if(step){
      const dur = STAIRS_STEP_ANIM_MS * (step.slow ? 1.6 : 1);
      if(ts - stairs.stepStartTs >= dur){
        ...
        stairs.playIndex++;
        stairs.stepStartTs = ts;
        if(stairs.playIndex >= stairs.steps.length) finishStairsRun();
      }
    }
    drawStairsCanvas();
  }
