...
// Writes synchronously, immediately, every time — this used to be debounced
// 250ms behind a setTimeout, which meant a restart landing inside that
// window could lose an ALREADY-PROCESSED bet/cashout write (not just an
// in-flight round), since the timer never got the chance to fire. Balance
// correctness matters more here than shaving a few disk writes.
function saveUsers() {
  try {
...
// now survive a restart instead of resetting to empty every time Bonto
...
loadGameState(); // restores history/stairsLeaderboard/day + refunds any leftover unresolved stake from before a hard kill
...
// actually exits.
function refundUnresolvedBetsOnShutdown() {
...
function gracefulShutdown(signal) {
  ...
  try { refundUnresolvedBetsOnShutdown(); } catch (e) { console.error('[shutdown] refund failed:', e); }
  process.exit(0);
}
