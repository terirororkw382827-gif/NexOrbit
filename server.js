...
// =====================================================================
// PLINKO
// Real Matter.js board: a proper Galton triangle of round static pegs
// (row r has r+1 pegs, r=0..PLINKO_ROWS-1), a golden ball dropped from
// dead center, and 11 slot dividers that line up exactly with the bottom
// row of pegs so there's no dead space between them. Which slot the ball
// lands in is decided entirely by the real collision solver — same
// "physics decides, server just gets told the result" pattern already
// used for Stairs.
//
// Multiplier layout is deliberately the OPPOSITE of what it might look
// like at first glance: 0x sits alone in the CENTER, not the edges. With
// low-restitution pegs (see pegOpts below) the ball behaves like a real
// Galton board — each peg only nudges it slightly left/right, so after
// PLINKO_ROWS independent nudges it follows a binomial distribution that's
// heavily weighted toward the middle column. That makes the center the
// MOST likely landing spot and the edges the LEAST likely — so the payout
// table has to mirror that (low in the middle, high at the edges) or the
// board would pay out constantly. This is the same shape every real
// Plinko board (arcade or casino) uses, for the same reason.
// =====================================================================
const PLINKO_MIN_BET = 1;
const PLINKO_MAX_BET = 1000;
const PLINKO_ROWS = 10;                 // pegs rows -> PLINKO_ROWS+1 = 11 slots
const PLINKO_SLOTS = PLINKO_ROWS + 1;
// NOTE on the values below: with restitution this low the ball follows a
// close-to-binomial(10, 0.5) distribution (10 pegs rows it has to clear),
// so the two slots next to center (index 3/7) land ~11.7% of the time EACH
// and the two after that (index 4/6, right next to the 0) land ~20.5% of
// the time EACH — over 60% of all drops combined. Those two "common" tiers
// are what actually decide the payout rate, not the rare 10x edges.
// Current table computes to ~113.7% RTP under that idealized 50/50 model —
// still above 100%, i.e. still a net loss for the house on average.
...
// One entry per ball currently in flight — { body, touchingSlot,
// wasTouching, settled, settleStartTs, bet }. Replaces the old single
// plinkoBall/plinkoDropping/... globals now that multiple balls can be
// dropped back-to-back without waiting for the previous one to land.
let plinkoBalls = [];
let plinkoAccMs = 0;
let plinkoLastTs = 0;
...
function plinkoDropOne(bet){
  ws.send(JSON.stringify({type:'plinko_bet', amount: bet}));

  // Strictly centered drop, with a tiny random jitter so repeated drops
  // don't all take the exact same path off the apex peg every time.
  const jitter = (Math.random()-0.5)*6;
  // restitution 0 here too — Matter.js resolves a collision's bounce using
  // the HIGHER of the two bodies' restitution values, so even with every
  // peg/wall/floor zeroed out, a bouncy ball would still override them and
  // bring the ricochet right back. Zero on both sides is what actually
  // kills all bounce.
  ...
  const state = { body, touchingSlot:null, wasTouching:false, settled:false, settleStartTs:0, bet };
...
function finishPlinkoDrop(state){
  ...
  ws.send(JSON.stringify({type:'plinko_result', payout, mult, bet: state.bet}));
...
async function saveGameState() {
  const payload = {
    ...
    leftoverBets: bets, leftoverStairsBets: pendingStairsBets, leftoverPlinkoBets: pendingPlinkoBets
  };
  if (mongoDb) {
    try {
      await mongoDb.collection('meta').updateOne(
...
// key -> array of amounts staked, one entry per ball still in flight (the
// client can drop several balls from one throw at once — see plinko_bet
// below), shifted off in FIFO order as each plinko_result comes back in.
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (raw) => {
    ...
    // ----- PLINKO -----
    // Same authoritative-balance-only pattern as Stairs: the board and
    // ball are simulated client-side with real Matter.js physics, the
    // server just trusts which slot it reported and applies the payout.
    // One extra guard here that Stairs doesn't have: the max multiplier
    // is a known fixed constant (10x), so a payout claiming more than
    // 10x the staked amount is provably impossible and gets rejected
    // outright rather than trusted.
    //
    // The client can drop several balls from one throw at once (1/3/5/10),
    // each with its own plinko_bet followed later by its own plinko_result
    // — so pendingPlinkoBets holds a queue (array) per user rather than a
    // single amount, and each result pops one entry off the front.
    if (msg.type === 'plinko_bet') {
      ...
      user.balance -= amount;
      await saveUsers();
      if (!Array.isArray(pendingPlinkoBets[authed])) pendingPlinkoBets[authed] = [];
      pendingPlinkoBets[authed].push(amount);
      await saveGameState();
      sendTo(ws, { type: 'balance', balance: user.balance });
    }
    ...
    if (msg.type === 'plinko_result') {
      const payout = Number(msg.payout);
      const queue = pendingPlinkoBets[authed];
      const stakedAmount = Array.isArray(queue) && queue.length ? queue.shift() : undefined;
      if (Array.isArray(queue) && queue.length === 0) delete pendingPlinkoBets[authed];
      if (!Number.isFinite(payout) || payout < 0) return;
      if (Number.isFinite(stakedAmount) && payout > stakedAmount * 10 + 0.01) return; // 10x is the known max — anything above is impossible
      user.balance += payout;
      await saveUsers();
      await saveGameState();
      sendTo(ws, { type: 'balance', balance: user.balance });
...
async function refundUnresolvedBetsOnShutdown() {
  ...
  for (const key of Object.keys(pendingPlinkoBets)) {
    if (users[key]) {
      const v = pendingPlinkoBets[key];
      const total = Array.isArray(v) ? v.reduce((a, b) => a + b, 0) : (Number(v) || 0);
      users[key].balance += total;
    }
  }
  await saveUsers();
