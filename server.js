...
const STAIRS_RENDER_AHEAD = 9;     // how many upcoming steps to keep generated/drawn/physical
...
let stairsSettleStartTs = 0;
let stairsWasTouching = false;

Events.on(stairsEngine, 'collisionStart', (ev) => {
  for(const pair of ev.pairs){
    ...
    if(stepBody && cubeInvolved){
      ...
      stairsSettleStartTs = performance.now();
    }
  }
});
...
function stairsEnsureGenerated(uptoIndex){
  while(!stairs.ranOut && stairs.steps.length <= uptoIndex){
    ...
    if(next){
      ...
      const base = stairsCumulativeXY(stairs.steps, i);
      const w = STAIRS_STEP_W * next.widthMult;
      stairs.steps.push(next);
      stairsStepBodies[i] = stairsCreateStepBody(next, i, base.x, base.y, w);
    } else {
      stairs.ranOut = true;
    }
  }
...
function stairsPhysicsStep(dtMs){
  ...
  while(stairsAccMs >= STAIRS_FIXED_DT){
    ...
    if(stairsTouchingStepIndex === null && stairs.ranOut && !stairsStepBodies.length){
