...
const STAIRS_HOP_VX_BASE = 7.5;    // px/step — horizontal launch speed at perfect aim+power
const STAIRS_HOP_VY0_BASE = 6.2;   // px/step — upward launch speed at perfect aim+power (arcs up, then gravity brings it down)
...
function newStairsGen(){
  return {
    longRemain:0, longMult:1, shortRemain:0, shortMult:1, idx:0
  };
}
...
function stairsGenNext(g){
  ...
  if(isWide){
    if(Math.random() < 0.65){
      ...
      if(effect === 'lengthen25'){ g.longRemain = 25; g.longMult = 1 + 0.5*tierEffMult; }
      // shrink/heavy/slow are applied as real physics changes to the live
      // cube body the instant this step is reached — see stairsApplyStepEffect.
    } else {
      special = 'green';
      effect = BONUSES[Math.floor(Math.random()*BONUSES.length)];
      if(effect === 'shorten10'){ g.shortRemain = 10; g.shortMult = Math.max(0.15, 1 - 0.5/tierEffMult); }
      // grow/speed/springy: same as above, applied physically on arrival.
    }
  }
...
function startStairsFlight(){
  ...
  // horizontal component of a launch velocity. This "quality" is now used
  // for exactly one thing: the strength of the single real launch below —
  // it no longer buys a step budget, because there is no step budget.
  const angleQuality = Math.max(0, Math.cos(stairs.angle * Math.PI/180));
  const powerQuality = Math.max(0, 1 - Math.abs(stairs.power - 50)/50);
  const throwQuality = angleQuality * powerQuality; // 0..1
  ...
  stairsLaunchCube(throwQuality);
}

// The ONE real launch for this run — a genuine initial velocity handed to
// the Matter body, scaled by how good the aim/power throw was. No target
// position, no duration, and (unlike before) no second call later: after
// this the cube is entirely on its own, and everything about where it
// goes and where it finally rests is decided by the engine's own collision
// solver against the real step bodies (see collisionStart above and
// stairsPhysicsStep below).
function stairsLaunchCube(throwQuality){
  const f = STAIRS_MIN_LAUNCH_FRAC + (1 - STAIRS_MIN_LAUNCH_FRAC) * Math.min(1, Math.max(0, throwQuality));
  Body.setVelocity(stairsCubeBody, {
    x: STAIRS_HOP_VX_BASE * f,
    y: -STAIRS_HOP_VY0_BASE * f // negative = upward, gravity pulls it back down
  });
...
function stairsApplyStepEffect(step){
  ...
  // lengthen25 / shorten10 only change the width of upcoming generated
  // steps (already handled in stairsGenNext) — no body change needed here.
}
...
function stairsAdvancePlayIndex(){
  while(stairsTouchingStepIndex !== null && stairs.playIndex <= stairsTouchingStepIndex){
