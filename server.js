...
<div class="app" id="app">
  ...
  <div class="view" id="viewPlinko">
    <div class="stairs-info">Ставка: от <b>1</b> до <b>1000</b>. Шар падает через колышки и приземляется в один из 13 множителей внизу.</div>
...
// dead center, and 13 slot dividers that line up exactly with the bottom
...
const PLINKO_ROWS = 12;                 // pegs rows -> PLINKO_ROWS+1 = 13 slots
const PLINKO_SLOTS = PLINKO_ROWS + 1;
const PLINKO_MULTS = [0,0,1.5,2,3,5,10,5,3,2,1.5,0,0];
const PLINKO_SLOT_COLORS = ['#3a3a45','#3a3a45','#e0507a','#ff3d5a','#ff6b35','#ff9f1c','#ffd23f','#ff9f1c','#ff6b35','#ff3d5a','#e0507a','#3a3a45','#3a3a45'];
...
function plinkoBuildBoard(){
  ...
  for(const b of plinkoBumperBodies) World.remove(plinkoWorld, b);
  plinkoPegBodies = []; plinkoWallBodies = []; plinkoFloorBodies = []; plinkoBumperBodies = [];
  ...
  const pegOpts = { isStatic:true, restitution:0.55, friction:0.05, label:'plinko_peg' };
  ...
  const outerWallOpts = { isStatic:true, restitution:0.3, friction:0.05, label:'plinko_wall' };
  ...
  // logic the two side bumpers use, so it genuinely kicks the ball back
  // down instead of just passively bouncing it), but it's pushed into
  // plinkoWallBodies (not plinkoBumperBodies) so the wall-drawing loop
  // below picks it up and paints it the exact same translucent white as
  // the side walls — not the black/yellow bumper style.
  //
  ...
  // Bumpers — big, strongly-repelling half-circles fixed flush to the
  // inner face of each outer wall (centered exactly ON the wall's inner
  // edge, so the embedded half sits behind solid wall geometry the ball
  // can never reach anyway, and only the round half genuinely protrudes
  // into the play area — they don't float free of the wall). Real
  // pinball-style kick, not just a bouncy surface: see PLINKO_BUMPER_KICK /
  // plinkoMarkBumperHit above, which actively shoves the ball away on
  // contact on top of Matter's own collision response.
  const bumperR = slotW*0.55;
  const bumperY = topY + 8.5*rowGapY; // row 8.5, same unit the peg rows use (was ~row 7)
  const bumperOpts = { isStatic:true, restitution:1.1, friction:0.02, label:'plinko_bumper' };
  plinkoBumperBodies.push(
    Bodies.circle(marginX + outerWallThickness/2, bumperY, bumperR, bumperOpts),
    Bodies.circle(marginX + boardW - outerWallThickness/2, bumperY, bumperR, bumperOpts)
  );
  World.add(plinkoWorld, plinkoBumperBodies);
  ...
  const dividerOpts = { isStatic:true, restitution:0.3, friction:0.05, label:'plinko_wall' };
  ...
  plinkoBoard = { w, h, marginX, boardW, slotW, centerX, topY, rowGapY, bottomOfPegsY, slotAreaH, pegR, ballR, wallTopY, bumperR, bumperY, outerWallThickness };
}
...
function plinkoDropOne(bet){
  ...
  const body = Bodies.circle(plinkoBoard.centerX + jitter, plinkoBoard.topY - plinkoBoard.rowGapY*0.8, plinkoBoard.ballR, {
    restitution: 0.48, friction: 0.05, frictionAir: 0.0005, label:'plinko_ball',
...
function drawPlinkoCanvas(){
  ...
  // bumpers — black half-circles with a yellow outline, flush against the
  // outer walls (only the half that actually protrudes into the play area
  // is drawn, matching the physical body's reachable half exactly)
  for(let i=0; i<plinkoBumperBodies.length; i++){
    const bump = plinkoBumperBodies[i];
    const isLeft = i === 0;
    const startAngle = isLeft ? -Math.PI/2 : Math.PI/2;
    const endAngle = isLeft ? Math.PI/2 : Math.PI*1.5;
    plinkoCtx.beginPath();
    plinkoCtx.moveTo(bump.position.x, bump.position.y);
    plinkoCtx.arc(bump.position.x, bump.position.y, b.bumperR, startAngle, endAngle);
    plinkoCtx.closePath();
    plinkoCtx.fillStyle = '#0a0a0a';
    plinkoCtx.fill();
    plinkoCtx.strokeStyle = '#ffd23f';
    plinkoCtx.lineWidth = Math.max(2, b.bumperR*0.12);
    plinkoCtx.stroke();
  }
