...
const STAIRS_COLOR_CHANCES = { rainbow: 0.005, diamond: 0.05, gold: 0.10, black: 0.01, orange: 0.01, lime: 0.01 }; // sums to 0.185, remaining 0.815 is a normal step (or a wide complication/bonus step)
const STAIRS_COLOR_BONUS = { gold: 0.05, diamond: 0.10, rainbow: 0.50, black: -0.10 }; // total multiplier contribution of that step (replaces the usual +0.01) — orange/lime aren't in here, they scale the running total instead (see stairsApplyStepEffect)
...
function stairsRollColor(){
  const r = Math.random();
  let acc = 0;
  for(const c of ['rainbow','diamond','gold','black','orange','lime']){
