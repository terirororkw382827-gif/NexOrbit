...
const PLINKO_MULTS = [10,5,3,2,0.7,0,0.7,2,3,5,10];
...
// Bumpers don't just passively bounce the ball (a real arcade/pinball
// bumper actively KICKS it, it doesn't just conserve energy like a wall
// would) — on first contact this adds a strong push straight out along
// the line from the bumper's center through the ball, on top of whatever
// Matter's own collision response already did. collisionStart only (not
// collisionActive) so a lingering graze doesn't re-kick every single
// physics step.
const PLINKO_BUMPER_KICK = 20;
function plinkoMarkBumperHit(ev){
  for(const pair of ev.pairs){
    const bumper = pair.bodyA.label==='plinko_bumper' ? pair.bodyA : (pair.bodyB.label==='plinko_bumper' ? pair.bodyB : null);
    const ball = pair.bodyA.label==='plinko_ball' ? pair.bodyA : (pair.bodyB.label==='plinko_ball' ? pair.bodyB : null);
    if(bumper && ball){
      let dx = ball.position.x - bumper.position.x;
      let dy = ball.position.y - bumper.position.y;
      const dist = Math.sqrt(dx*dx + dy*dy) || 1;
      dx /= dist; dy /= dist;
      Body.setVelocity(ball, {
        x: ball.velocity.x + dx*PLINKO_BUMPER_KICK,
        y: ball.velocity.y + dy*PLINKO_BUMPER_KICK
      });
    }
  }
}
Events.on(plinkoEngine, 'collisionStart', plinkoMarkBumperHit);
...
function plinkoBuildBoard(){
  ...
  const pegR = Math.max(2, slotW*0.07);
  const ballR = Math.max(4, slotW*0.22);
  ...
  const pegOpts = { isStatic:true, restitution:0.25, friction:0.1, label:'plinko_peg' };
  for(let r=0; r<PLINKO_ROWS; r++){
    if(r === 0) continue; // apex single-peg row removed — every other row keeps its exact original position
    const y = topY + r*rowGapY;
    for(let k=0; k<=r; k++){
      const x = centerX + (k - r/2)*slotW;
  ...
  const outerWallOpts = { isStatic:true, restitution:0.15, friction:0.05, label:'plinko_wall' };
  ...
  plinkoWallBodies.push(
    Bodies.rectangle(centerX, roofY, boardW, roofThickness, {
      isStatic:true, restitution:1.1, friction:0.02, label:'plinko_bumper'
    })
  );
  ...
  const dividerOpts = { isStatic:true, restitution:0.15, friction:0.05, label:'plinko_wall' };
  ...
  for(let i=0; i<PLINKO_SLOTS; i++){
    ...
    const body = Bodies.rectangle(x0 + slotW/2, floorY, slotW, 8, {
      isStatic:true, friction:0.4, restitution:0.05, label:'plinko_floor', slotIndex:i
    });
...
function plinkoDropOne(bet){
  ...
  // restitution kept low here too — Matter.js resolves a collision's
  // bounce using the HIGHER of the two bodies' restitution values, so a
  // bouncy ball would override the low-restitution pegs/walls above and
  // we'd be right back to chaotic ricocheting.
  const body = Bodies.circle(plinkoBoard.centerX + jitter, plinkoBoard.topY - plinkoBoard.rowGapY*0.8, plinkoBoard.ballR, {
    restitution: 0.2, friction: 0.05, frictionAir: 0.0005, label:'plinko_ball',
