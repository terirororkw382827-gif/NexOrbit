...
let stairs = {
  ...
  flightState:null, x:0, y:0, vx:0, vy:0, groundY:0, anchorX:0,
...
const STAIRS_HOP_TIME = 0.22;      // s — nominal time to clear one normal-width step
...
// Solves the initial velocity (vx, vy) so that, integrated under real
// gravity, the cube covers exactly this step's (dx, dy) displacement in
// about STAIRS_HOP_TIME seconds — the same math as aiming a real
// projectile at a known target and duration.
function launchTowardsStep(stepIdx){
  const step = stairs.steps[stepIdx];
  const dx = STAIRS_STEP_W * step.widthMult;
  const dy = STAIRS_STEP_H;
  const T = STAIRS_HOP_TIME * (step.slow ? 1.6 : 1) / (step.springy ? 1.15 : 1);
  stairs.vx = dx / T;
  stairs.vy = (dy - 0.5*STAIRS_GRAVITY*T*T) / T;
  stairs.anchorX = stairs.x + dx;   // exact expected landing x, used to kill drift
  stairs.groundY = stairs.y + dy;   // this hop's landing surface, in world coords
  stairs.flightState = 'air';
}

// One physics tick (dt in seconds): integrates real gravity every frame,
// and on ground contact applies restitution (bounce) + friction, decaying
// bounces until the cube settles — same treatment as a real bouncing ball.
function stairsPhysicsStep(dt){
  if(stairs.flightState === 'air' || stairs.flightState === 'bounce'){
    ...
    if(stairs.y >= stairs.groundY){
      stairs.y = stairs.groundY;
      if(Math.abs(stairs.vy) > STAIRS_BOUNCE_MIN_VEL){
        stairs.vy = -stairs.vy * STAIRS_RESTITUTION; // bounce
        stairs.vx *= STAIRS_FRICTION;                // friction bleeds speed each contact
        stairs.flightState = 'bounce';
      } else {
        stairs.vy = 0; stairs.vx = 0;
        stairs.x = stairs.anchorX; // snap to the exact anchor — bounces are cosmetic drift only
...
function drawStairsCanvas(){
  ...
  const step = stairs.steps[Math.min(stairs.playIndex, stairs.steps.length-1)];
