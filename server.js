...
const STAIRS_GAP_CHANCE = 0.0075;  // 0.75% — this step has NO floor at all (a real hole in the
                                    // geometry); falling into it while over its span is instant death
...
const STAIRS_COLOR_LABELS = {
  ...
  sticky: 'Липкая ступень! Импульс погашен', booster: 'Ускоритель! Куб рвануло вперёд'
};
...
function stairsGenNext(g){
  ...
  // Gap/wall obstacles take priority over everything else below — a step
  // that's a hazard doesn't also roll wide/red/green/color on top of it.
  const hazardRoll = Math.random();
  let kind = 'normal';
  if(hazardRoll < STAIRS_GAP_CHANCE) kind = 'gap';
  else if(hazardRoll < STAIRS_GAP_CHANCE + STAIRS_WALL_CHANCE) kind = 'wall';

  if(kind !== 'normal'){
    return {
      index: idx, special: null, effect: null,
      multiplier: idx/100,
      widthMult, kind, color: null
    };
  }
