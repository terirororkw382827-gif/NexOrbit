...
function drawStairsCanvas(){
  ...
  if(!stairs.steps.length){
    ...
    // checkered starting pad + cube at the exact position/camera anchor the
    // real flight will begin from (see stairsCreateCubeBody: (0,-20)), so
    // the screen isn't just empty while the player is aiming, and there's
    // no visual jump the instant the throw actually launches.
    const camX = 0 - w*0.30, camY = -20 - h*0.30;
    ...
    const size = 22;
    stairsCtx.save();
    stairsCtx.translate(0 - camX, -20 - camY);
