...
let plinkoTouchingSlot = null;
let plinkoWasTouching = false;
let plinkoSettleStartTs = 0;
...
function plinkoMarkTouch(ev){
  for(const pair of ev.pairs){
    ...
    const ballInvolved = pair.bodyA.label==='plinko_ball' || pair.bodyB.label==='plinko_ball';
    if(floorBody && ballInvolved){
      plinkoTouchingSlot = floorBody.slotIndex;
      plinkoWasTouching = true;
    }
  }
}
...
function plinkoBuildBoard(){
  ...
  // Was h*0.08 — bumped up to leave real headroom above the peg triangle
  // for the roof bumper below (without this the ball's own spawn point
  // sat right where the roof needed to go, with zero clearance).
  const topY = h*0.16;
  ...
  // Position is clamped so it always has real clearance below it (never
  // overlapping the ball's spawn point) and never renders above y=0.
  const spawnY = topY - rowGapY*0.8; // matches the ball's actual drop start in onPlinkoBtn
  const roofThickness = outerWallThickness;
  const roofYIdeal = topY - rowGapY*1.3;
  const roofYMax = spawnY - ballR - roofThickness/2 - 3;
  const roofY = Math.max(roofThickness/2 + 2, Math.min(roofYIdeal, roofYMax));
  plinkoWallBodies.push(
...
function onPlinkoBtn(){
  ...
  if(plinkoBall) World.remove(plinkoWorld, plinkoBall);
  ...
  plinkoBall = Bodies.circle(plinkoBoard.centerX + jitter, plinkoBoard.topY - plinkoBoard.rowGapY*0.8, plinkoBoard.ballR, {
    restitution: 0.48, friction: 0.05, frictionAir: 0.0005, label:'plinko_ball'
  });
  World.add(plinkoWorld, plinkoBall);

  plinkoDropping = true;
  plinkoTouchingSlot = null;
  plinkoWasTouching = false;
  plinkoSettled = false;
  plinkoAccMs = 0;
  plinkoCurrentBet = bet;
  plinkoBtn.disabled = true;
  plinkoStatusLine.textContent = 'Шар летит...';
}
let plinkoCurrentBet = 0;
...
function plinkoPhysicsStep(dtMs){
  ...
  while(plinkoAccMs >= PLINKO_FIXED_DT){
    ...
    if(plinkoTouchingSlot !== null){
      const speed = Vector.magnitude(plinkoBall.velocity);
      if(plinkoWasTouching && speed < PLINKO_SETTLE_SPEED){
        if(!plinkoSettled){ plinkoSettled = true; plinkoSettleStartTs = performance.now(); }
      } else if(plinkoWasTouching){
        plinkoSettled = false;
      }
    }
  }
  if(plinkoSettled && performance.now() - plinkoSettleStartTs >= PLINKO_SETTLE_PAUSE){
    finishPlinkoDrop();
  }
}
...
function finishPlinkoDrop(){
  ...
  plinkoSettled = false;
  const slot = plinkoTouchingSlot != null ? plinkoTouchingSlot : Math.floor(PLINKO_SLOTS/2);
  const mult = PLINKO_MULTS[slot];
  const payout = Math.round(plinkoCurrentBet * mult * 100) / 100;
  ws.send(JSON.stringify({type:'plinko_result', payout, mult, bet: plinkoCurrentBet}));
  plinkoBtn.disabled = false;
...
function drawPlinkoCanvas(){
  ...
  // ball — golden with a glossy highlight
  if(plinkoBall){
    const x = plinkoBall.position.x, y = plinkoBall.position.y, r = b.ballR;
...
function plinkoFrame(ts){
  ...
  plinkoLastTs = ts;
  if(plinkoDropping && dt > 0) plinkoPhysicsStep(dt*1000);
